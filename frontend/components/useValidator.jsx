...
const useValidation = (initialState) => {
  ...
  const validateAllFields = (fields, isRestaurant) => {
    if (isRestaurant) {
      // Step 2
      const equipmentValid = fields.equantity.every(
        (item) =>
          (!item.name.field &&
            !item.quantity.field &&
            !item.option_value.field) || // Empty item
          (item.name.validate === "true" &&
            item.quantity.validate === "true" &&
            (item.validOptions.length === 0 ||
              item.option_value.validate === "true")) // Option required only if options exist
      );
      const areasValid = fields.areas.every(
        (area) =>
          (!area.name.field &&
            !area.square_feet.field &&
            !area.floor_type.field) || // Empty area
